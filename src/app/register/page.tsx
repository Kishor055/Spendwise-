...
import { Wallet, Loader2, User, Mail, Lock, Phone, ChevronRight } from 'lucide-react';
            ...
            <form onSubmit={handleRegister} className="space-y-6">
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input placeholder="E.g. Kishor Kakde" value={name} onChange={(e) => setName(e.target.value)} required className="h-14 rounded-2xl glass border-white/5 pl-14 text-sm font-bold placeholder:text-white/10" />
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input type="tel" placeholder="9876543210" value={phone} onChange={(e) => setPhone(e.target.value)} required className="h-14 rounded-2xl glass border-white/5 pl-14 text-sm font-bold placeholder:text-white/10" />
                </div>
              </div>
              ...
              <Button type="submit" className="w-full h-16 rounded-2xl text-xs font-black uppercase tracking-[0.3em] bg-accent text-accent-foreground hover:bg-accent/80 shadow-xl shadow-accent/20 group transition-all" disabled={loading}>
                {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <>Establish Nexus <ChevronRight className="ml-2 h-4 w-4 group-hover:translate-x-1 transition-transform" /></>}
              </Button>
            </form>
          </CardContent>

          <div className="p-8 pt-0 text-center">
            <p className="text-[10px] text-white/30 font-black uppercase tracking-[0.2em]">Registered? <Link href="/login" className="text-accent hover:text-primary transition-colors">Resume Session</Link></p>
          </div>
        </Card>
      </motion.div>
    </div>
